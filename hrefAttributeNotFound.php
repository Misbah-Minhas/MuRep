    ...
    <ul class="nav navbar-nav navbar-right">
      <li><a href="index.php"><span class="glyphicon glyphicon-log-in"></span> Home</a></li>
		...
		<?php
					...
					$operator = "HrefBaseLocatorNotFound";
					if($operator == "HrefBaseLocatorNotFound")
					{
						$conn = db ();
						$sql= "TRUNCATE hrefattributenotfound";
							...
							$dom->loadHTML($html);
							$inputs = $dom->getElementsByTagName('a');
								foreach ($inputs as $input){
										$typeText=$input->getAttribute('href');
									//	if($typeText=="text")
									//	{
										$OriginalCode=$input->getAttribute('href');
										...
										$RandomNum="#";
										$input->setAttribute('href', $RandomNum);
...
$sql= "INSERT INTO HrefAttributeNotFound (OriginalCode,mutantV1) VALUES ('".$OriginalCode."','".$MRandomNum."')";
$conn = db ();
if ($conn->query($sql) === TRUE) {
					...
					<div class="col-md-4 col-lg-4 col-sm-4">
						<div class="mutationSummary" style="overflow: scroll;height:500px">
							<p>Mutation Summary</p>
							 <?php
 $page_query = "SELECT * FROM HrefAttributeNotFound";  
...
function mf (data1, data2 ){
			...
			var data3=document.getElementById("PathName").value;
			  ...
			  var data4="hrefattributenotfound";
