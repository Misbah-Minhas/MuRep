    ...
    <ul class="nav navbar-nav navbar-right">
      <li><a href="index.php"><span class="glyphicon glyphicon-log-in"></span> Home</a></li>
		...
		<?php
					...
					$operator = "TypeAttributeChange";
					if($operator == "TypeAttributeChange")
					{
						$conn = db ();
							...
							libxml_use_internal_errors(true);
									...
									$dom = new DOMDocument;
										...
										foreach ($inputs as $input){
											$intype=$input->getAttribute('type');
											
												if($intype=='checkbox'){
													$OriginalCode=$input->setAttribute('type','radio');
											
												}
												$OriginalCode = $input->ownerDocument->saveHtml($input);
												$OriginalCode=htmlspecialchars($OriginalCode);	
														}
...
$sql= "INSERT INTO typeattributechange (OriginalCode,mutantV1) VALUES ('".$OriginalCode."','".$RemoveField."')";
$conn = db ();
if ($conn->query($sql) === TRUE) {
					...
					<div class="col-md-4 col-lg-4 col-sm-4">
						<div class="mutationSummary" style="overflow: scroll;height:500px">
							<p>Mutation Summary</p>
							 <?php
 $page_query = "SELECT * FROM typeattributechange";  
...
function mf (data1, data2 ){
			...
			var data3=document.getElementById("PathName").value
			  ...
			  var data4="typeattributechange";
    load_data(data1,data2,data3,data4);  
      function load_data(page)  
      {  
           $.ajax({ 
                url:"mutateHTMLWTR.php",  
                method:"POST",  
