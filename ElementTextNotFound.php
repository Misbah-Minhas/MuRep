    ...
    <ul class="nav navbar-nav navbar-right">
      <li><a href="index.php"><span class="glyphicon glyphicon-log-in"></span> Home</a></li>
		...
		<?php
					...
					$operator = "ElementTextNotFound";
					if($operator == "ElementTextNotFound")
					{
						$conn = db ();
						$sql= "TRUNCATE elementtextnotfound";
							...
							$dom->loadHTML($html);
							$inputs = $dom->getElementsByTagName('p');
							foreach ($inputs as $input){
										//$OriginalCode=$input->getAttribute('id');
										...
										$text3=$input->textContent;
										$ChangeContent = $input->ownerDocument->saveHtml($input);
										$ChangeContent=base64_encode($ChangeContent);
									
									//	$ChangeContent=htmlspecialchars($ChangeContent);
									
										addtodb($OriginalCode,$ChangeContent);		
										}				
					}
					else {
				...
				?>
<?php
function addtodb($OriginalCode,$RemoveField){
$sql= "INSERT INTO elementtextnotfound (OriginalCode,mutantV1) VALUES ('".$OriginalCode."','".$RemoveField."')";
$conn = db ();
if ($conn->query($sql) === TRUE) {
					...
					<div class="col-md-4 col-lg-4 col-sm-4">
						<div class="mutationSummary" style="overflow: scroll;height:500px">
							<p>Mutation Summary</p>
							 <?php
 $page_query = "SELECT * FROM elementtextnotfound";  
 ...
 $result = mysqli_query($connect, $page_query);
  ...
  while($row = mysqli_fetch_array($result))  
	...
	///	echo $temp9;
			...
			echo "<h4>Mutants</h4>";
			echo "<span class='ClickMutants'><p onclick=mf(".$row['Sr'].",'mutantV1');><img src='images/bug.jpg' style='width:6%;'/>  Remove Input field </p></span>";
