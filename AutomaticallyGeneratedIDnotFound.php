    ...
    <ul class="nav navbar-nav navbar-right">
      <li><a href="index.php"><span class="glyphicon glyphicon-log-in"></span> Home</a></li>
		...
		<?php
					...
					$operator = "AutomaticallyGeneratedIDnotFound";
					if($operator == "AutomaticallyGeneratedIDnotFound")
					{
						$conn = db ();
						$sql= "TRUNCATE idnotfound";
							...
							$inputs = $dom->getElementsByTagName('input');
								foreach ($inputs as $input){
										$typeText=$input->getAttribute('type');
										if($typeText=="text")
										{
										...
										$input->setAttribute('id', $RandomNum);
										$MRandomNum = $input->ownerDocument->saveXML($input);
										$MRandomNum=base64_encode($MRandomNum);
										
										//$MRandomNum=htmlspecialchars($MRandomNum);
										addtodb($OriginalCode,$MRandomNum);
										}
										
										if ($typeText!="text" && $typeText!="text" ) {
											//header("Location: http://localhost/mutationTesting/errors.php/myFunc");
										//	header("Location:http://localhost/mutationTesting/errors.php/?action=myFunc&param=123");
										}
										}					
					}
					else {
...
$sql= "INSERT INTO idnotfound (OriginalCode,mutantV1) VALUES ('".$OriginalCode."','".$MRandomNum."')";
$conn = db ();
if ($conn->query($sql) === TRUE) {
					...
					<div class="col-md-4 col-lg-4 col-sm-4">
						<div class="mutationSummary" style="overflow: scroll;height:500px">
							<p>Mutation Summary</p>
							 <?php
 $page_query = "SELECT * FROM idnotfound";  
 ...
 $result = mysqli_query($connect, $page_query);
  ...
  while($row = mysqli_fetch_array($result))  
	...
	///	echo $temp9;
			...
			echo "<h4>Mutants</h4>";
			echo "<span class='ClickMutants'><p onclick=mf(".$row['Sr'].",'mutantV1');><img src='images/bug.jpg' style='width:6%;'/>  Change ID attribute </p></span>";
