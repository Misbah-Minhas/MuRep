    ...
    <ul class="nav navbar-nav navbar-right">
      <li><a href="index.php"><span class="glyphicon glyphicon-log-in"></span> Home</a></li>
		...
		<?php
					...
					$operator = "ClassAttributeNotFound";
					if($operator == "ClassAttributeNotFound")
					{
						$conn = db ();
						$sql= "TRUNCATE classattributenotfound";
							...
							foreach ($inputs as $node) {
								...
								$MutatedClass=$inputs[$i]->getAttribute('class');
								 if($MutatedClass!=""){
									...
									$MRandomNum = $inputs[$i]->ownerDocument->saveXML($inputs[$i]);
...
$sql= "INSERT INTO classattributenotfound (OriginalCode,mutantV1) VALUES ('".$OriginalCode."','".$MRandomNum."')";
$conn = db ();
if ($conn->query($sql) === TRUE) {
					...
					<div class="col-md-4 col-lg-4 col-sm-4">
						<div class="mutationSummary" style="overflow: scroll;height:500px">
							<p>Mutation Summary</p>
							 <?php
 $page_query = "SELECT * FROM classattributenotfound";  
 ...
 $result = mysqli_query($connect, $page_query);
  ...
  while($row = mysqli_fetch_array($result))  
	...
	///	echo $temp9;
			...
			echo "<h4>Mutants</h4>";
			echo "<span class='ClickMutants'><p onclick=mf(".$row['Sr'].",'mutantV1');><img src='images/bug.jpg' style='width:6%;'/>  Replace class attribute </p></span>";
...
function mf (data1, data2 ){
			...
			var data3=document.getElementById("PathName").value;
			  ...
			  var data4="classattributenotfound";
